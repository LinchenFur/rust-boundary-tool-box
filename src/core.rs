//! Core installation, launch, detection, and cleanup logic.
//!
//! This module deliberately owns filesystem mutations and process operations.
//! The Slint UI calls into `InstallerCore`, while all risky work such as
//! replacing files, restoring backups, killing processes, reading Steam
//! metadata, and validating online downloads stays centralized here.
...
/// Application version shown in the UI and written to install metadata.
pub const APP_VERSION: &str = "1.2.0";
/// Steam App ID for Boundary.
pub const APP_ID: &str = "1364020";
/// Shipping game executable used to validate a Binaries\Win64 directory.
pub const GAME_EXE: &str = "ProjectBoundarySteam-Win64-Shipping.exe";
/// Folder next to the executable used for logs and installer metadata.
pub const INSTALLER_FOLDER_NAME: &str = "installer_tool";
/// Metadata folder created inside the target Win64 directory.
...
/// Windows CREATE_NO_WINDOW flag for hidden helper processes.
pub const CREATE_NO_WINDOW: u32 = 0x0800_0000;
/// Nightly package used for the two ProjectRebound runtime binaries.
...
/// Files intentionally fetched online instead of embedded in payload.zip.
pub const PROJECT_REBOUND_ONLINE_FILES: &[&str] =
    &["Payload.dll", "ProjectReboundServerWrapper.exe"];
/// TCP ports required by the local login/game services.
pub const REQUIRED_TCP_PORTS: &[u16] = &[6969, 7777, 8000, 9000];
/// UDP ports required by the local game services.
pub const REQUIRED_UDP_PORTS: &[u16] = &[7777, 9000];
/// Port rows shown in the diagnostics UI.
...
// Generated by build.rs. It may be empty in source-only public builds, so
// validate_payload() must verify the required entries before installation.
const PAYLOAD_ZIP_BYTES: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/payload.zip"));

/// Type of a managed payload item.
...
/// File or directory owned by this installer inside Binaries\Win64.
...
/// Complete set of files/directories installed or removed by the toolbox.
...
// Old Python/mod package leftovers that are safe to clean because their names
// are known and scoped to the Boundary install tree.
...
/// Per-item install record persisted in state.json for exact uninstall/restore.
...
/// Window-topmost preferences persisted beside the install state.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
...
/// Full install metadata. This is the authoritative uninstall source.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
...
/// Minimal fallback metadata used when state.json is unavailable.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
...
/// Process row captured from sysinfo for UI diagnostics and cleanup.
...
/// Grouped runtime processes that belong to the selected Boundary install.
#[derive(Debug, Clone, Default)]
...
/// A process bound to a required local port.
...
/// UI-friendly status for one monitored port.
#[derive(Debug, Clone)]
...
/// Paths required to launch the community server and game.
#[derive(Debug, Clone)]
...
/// Directory discovery mode selected in the UI.
...
/// All metadata paths derived from one target Binaries\Win64 directory.
#[derive(Debug, Clone)]
...
/// Main service object used by the UI.
///
/// It is cheap to clone because the logger is reference-counted. Long-running
/// work is still executed on background threads by the UI layer.
#[derive(Clone)]
...
impl InstallerCore {
    /// Creates the core service and prepares the toolbox home/log directories.
    ...
    /// Sends a timestamped line through the UI/session logger.
    pub fn log(&self, message: impl Into<String>) {
        (self.logger)(format!("[{}] {}", now_text(), message.into()));
    }

    /// Human-readable payload source shown in logs and settings.
    pub fn payload_label(&self) -> &'static str {
        "内嵌载荷 + ProjectRebound 在线 Nightly"
    }

    /// Ensures all embedded payload entries are present before touching target files.
    ...
    /// Reads persisted topmost settings, falling back to safe defaults.
    ...
    /// Normalizes and stores the current topmost settings under metadata.
    ...
    /// Accepts either game root, ProjectBoundary root, or Binaries\Win64.
    pub fn normalize_selected_path(&self, raw: impl AsRef<Path>) -> Result<PathBuf> {
        normalize_selected_path(raw.as_ref())
    }

    /// Finds Boundary through Steam registry and library metadata.
    pub fn detect_steam_game_win64(&self) -> Result<(PathBuf, String)> {
        detect_steam_game_win64()
    }

    /// Lists existing Windows drive roots for the scan dialog.
    pub fn list_available_drives(&self) -> Vec<PathBuf> {
        list_available_drives()
    }

    /// Searches selected drives in parallel and returns the first valid Win64 directory.
    pub fn scan_drives_for_game(&self, drives: &[PathBuf]) -> Option<PathBuf> {
        scan_drives_for_game(drives, self.logger.clone())
    }

    /// Installs/updates the toolbox payload into the selected game directory.
    ///
    /// The online ProjectRebound package is downloaded and fully validated before
    /// any target file is replaced. Existing non-installer files are backed up
    /// once and tracked in state.json for later restoration.
    pub fn install(&self, target_win64: &Path, keep_topmost: bool, hotkey: &str) -> Result<String> {
        ...
        // Runtime files are loaded by the game/server, so stop related
        // processes before backing up or atomically replacing files.
        ...
        for item in MANAGED_ITEMS {
            ...
            if target_path.exists() && !installer_managed_before {
                // Only back up files that predate this installer. Files already
                // marked as installer-managed can be overwritten without
                // creating backup chains on every update.
            ...
            let source_path = if is_project_rebound_online_file(item.name) {
                ...
                // Embedded items are removed first because directories cannot be
                // replaced with file writes and may contain stale children.
            ...
            // State records are collected after writes so size/hash represent
            // the exact content that was installed.
    ...
    /// Removes installed files and restores backups when state.json is intact.
    pub fn uninstall(&self, target_win64: &Path) -> Result<String> {
        ...
        if let Some(state) = state {
            ...
            // Reverse order mirrors installation and avoids deleting parent
            // directories before child files have been processed.
        ...
        // markers.json is intentionally limited: it lets us remove known files
        // but refuses to restore anything because backup relationships are lost.
    ...
    /// Resolves all files needed by launch and reports missing paths together.
    ...
    /// Collects only Boundary-owned runtime processes for the selected target.
    ///
    /// Matching is intentionally scoped by executable path or command line
    /// containing the target directory. This avoids killing unrelated node.exe
    /// services that happen to run a file named index.js elsewhere.
    pub fn collect_runtime_processes(&self, target_win64: &Path) -> Result<RuntimeSnapshot> {
        ...
        for process in system.processes().values() {
            ...
            // Prefer exact executable paths, then fall back to process names
            // only when the command line proves the process belongs to this
            // Boundary install.
    ...
    /// Returns currently occupied required ports.
    pub fn collect_port_conflicts(&self) -> Result<Vec<PortConflict>> {
        collect_port_conflicts()
    }

    /// Builds fixed-order diagnostics rows for the UI port list.
    ...
    /// Terminates processes that occupy required ports.
    ...
    /// Terminates game, wrapper, login server, and topmost watcher processes.
    ...
    /// Best-effort pre-install process shutdown with post-kill verification.
    ...
    /// Starts login server, ProjectRebound wrapper, the game, and topmost watcher.
    ...
    /// Computes metadata locations for a selected installation.
    ...
    /// Copies an existing non-installer item into the per-install backup folder.
    fn backup_item(
        &self,
        source_path: &Path,
        backup_root: &Path,
    ...
    /// Writes the marker file used for limited cleanup if state.json is missing.
    ...
    /// Loads full install state. Missing file is Ok(None); corrupt JSON is an error.
    ...
    /// Loads minimal install markers. Missing file is Ok(None); corrupt JSON is an error.
...
/// UI/log timestamp in local time.
pub fn now_text() -> String {
    Local::now().format("%Y-%m-%d %H:%M:%S").to_string()
}

/// Compact ISO-like timestamp persisted in metadata.
pub fn iso_now() -> String {
    Local::now().format("%Y-%m-%dT%H:%M:%S").to_string()
}

/// Topmost target is currently fixed to the game window.
pub fn normalize_topmost_mode(_value: &str) -> String {
    DEFAULT_TOPMOST_MODE.to_string()
}

/// Parses truthy/falsy persisted strings for topmost toggles.
...
/// Validates and normalizes a user-entered or captured hotkey.
pub fn normalize_hotkey(value: impl AsRef<str>) -> Result<String> {
    Ok(parse_hotkey_text(value.as_ref())?.normalized)
}

/// Handles the hidden watcher subprocess command-line mode.
...
/// Keeps installer data beside the executable or under installer_tool.
...
/// Creates a directory tree and maps filesystem errors into anyhow.
fn ensure_dir(path: &Path) -> Result<()> {
    fs::create_dir_all(path)?;
    Ok(())
}

/// Creates a Windows command with no console window and no inherited stdio.
...
/// taskkill wrapper kept separate for testability and consistent flags.
fn hidden_taskkill_command() -> Command {
    hidden_command("taskkill")
}

/// Opens the build-time embedded payload archive.
fn open_payload_archive() -> Result<ZipArchive<Cursor<&'static [u8]>>> {
    ZipArchive::new(Cursor::new(PAYLOAD_ZIP_BYTES)).context("无法读取内嵌载荷")
}

/// Extracts one managed embedded file or directory into the target root.
...
/// Returns true for files sourced from the online ProjectRebound release.
...
/// Downloads the current ProjectRebound Nightly zip.
...
/// Preflights the online zip and returns every required file in memory.
///
/// Keeping bytes in memory before replacement prevents a partial install when
/// the proxy returns HTML, a corrupt zip, or a release missing one of the files.
...
/// Finds one required file anywhere in the ProjectRebound zip by basename.
...
/// Writes a validated ProjectRebound file to its final target path.
...
/// Writes to a sibling temp file and atomically replaces the final file.
fn replace_file_with_bytes(target_path: &Path, bytes: &[u8]) -> Result<()> {
    ...
    // MoveFileExW with WRITE_THROUGH gives stronger durability semantics than
    // remove+rename and prevents leaving a missing target on ordinary failures.
...
/// Builds a unique temp path beside the target so replacement stays same-volume.
...
/// Performs the Windows atomic replace.
...
/// Converts a Rust path into a null-terminated UTF-16 Windows string.
...
/// Streams a file into SHA-256 without loading large files fully into memory.
...
/// Collects size/hash/file-count metadata for installed records.
...
/// Reads optional JSON metadata while treating corrupt data as a blocking error.
...
/// Writes pretty JSON metadata, creating parent directories as needed.
...
/// Copies either a single file or a directory tree.
...
/// Collapses repeated blank lines after Engine.ini cleanup.
...
/// Removes known child entries from a directory and returns removed paths.
...
/// Cleans old one-click package leftovers that conflict with the Rust toolbox.
...
/// Removes stale OnlineSubsystem settings that break local community login.
fn clean_engine_ini(logger: Logger) -> Result<Option<PathBuf>> {
    ...
    // Rebuild the file section-by-section so unrelated user settings and
    // ordering survive the cleanup.
...
/// Lists drive roots that currently exist on Windows.
...
/// Depth-first scan for the game executable under one drive.
    ...
    while let Some(current) = stack.pop() {
        ...
        for entry in entries.flatten() {
                    ...
                    .is_some_and(|name| name.to_string_lossy().eq_ignore_ascii_case(GAME_EXE))
            {
                ...
                // Skip system folders that commonly return access errors and
                // cannot contain a Steam library in normal installs.
...
/// Scans multiple drives concurrently and stops workers after the first hit.
...
/// Verifies that a path is the exact Boundary Binaries\Win64 directory.
...
/// Resolves user-selected game root variants to Binaries\Win64.
...
/// Reads common Steam install roots from HKCU/HKLM registry keys.
...
/// Builds the candidate list of Steam libraryfolders.vdf files.
...
/// Parses Steam VDF library paths with a small targeted regex.
...
/// Reads the Boundary appmanifest install directory if the manifest exists.
...
/// Detects Boundary's Win64 path from Steam metadata.
...
/// Produces one compact process summary string for dialogs/logs.
...
/// Formats one process with PID and either exe path or command line.
...
/// Keeps long command lines readable in UI dialogs.
...
/// Deduplicates process IDs across runtime groups.
...
/// Formats occupied ports for confirmation dialogs and errors.
...
/// Reads OS socket tables and filters for ports required by the local server.
...
/// Appends conflict rows and tolerates sockets without process metadata.
fn append_conflicts(
    protocol: &str,
    port: u16,
    pids: &[u32],
...
/// Kills PIDs and treats any taskkill failure as a user-visible error.
...
/// Runs taskkill for each PID and returns detailed failures instead of hiding them.
...
/// Selects the most useful taskkill diagnostic text.
...
/// Derives executable paths from the selected Win64 directory.
...
/// Canonical lowercase path used for process matching.
